...
Если предполагается, что переменная может вернуть значение с пробелом, можно обернуть её в кавычки (как и всегда при 
передаче параметров командам), например, так: 
`"{{ .path }}"`
